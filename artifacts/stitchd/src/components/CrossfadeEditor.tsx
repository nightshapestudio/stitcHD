...
function PresetRow({ current, max, onSet }: { current: number; max: number; onSet: (v: number) => void }) {
  return (
...
export function CrossfadeEditor({ clip, outputDuration, onChange }: CrossfadeEditorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {outputDuration > 0 && clip.fadeIn + clip.fadeOut > outputDuration && (
        <p className="text-[8px] text-amber-400/80 uppercase tracking-[0.06em] leading-snug">
          Fades overlap — will be scaled to fit {outputDuration.toFixed(2)} s clip
        </p>
      )}
    </div>
  );
